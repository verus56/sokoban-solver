...
class Node:
    ...
    def isDeadLock(self):
        # Dead Lock coins
        wall_space_obstacle = Node.wall_space_obstacle
        robot_block = self.state.robot_block

        isHU = False
        isHD = False
        isVL = False
        isVR = False
        #avoir les coordonees des lignes DEADLOCK
        #HU HORIZONTAL UP   
        for i in range(len(wall_space_obstacle)):
            for j in range (len(wall_space_obstacle[0])):
                if (wall_space_obstacle[i][j] != ' '):
                    HU = i 
                    break
        
        #HD HORIZONTAL DOWN
        for i in range(len(wall_space_obstacle)-1,-1,-1):
            for j in range (len(wall_space_obstacle[0])):
                if (wall_space_obstacle[i][j] != ' '):
                    HD = i
                    break
        ...
        #VR VERTICAL RIGHT
        for j in range(len(wall_space_obstacle[0])-1,-1,-1):
            for i in range (len(wall_space_obstacle)):
                if (wall_space_obstacle[i][j] != ' '):
                    VR = j;  

        #verifer s'il y a pas de cibles dans une des deadlocks données
        #vérifier HU
        for j in range(len(wall_space_obstacle[0])):
            if (wall_space_obstacle[HU][j] == 'S'):
                isHU = False
        #verifier HD
        for j in range(len(wall_space_obstacle[0])):
            if (wall_space_obstacle[HD][j] == 'S'):
                isHD = False
        #verifier VL
        for i in range(len(wall_space_obstacle)):
            if (wall_space_obstacle[i][VL] == 'S'):
                isVL = False
        #verifier VR
        for i in range(len(wall_space_obstacle)):
            if (wall_space_obstacle[i][VR] == 'S'):
                isVR = False
        

        for i in range(len(wall_space_obstacle)):
            for j in range (len(wall_space_obstacle[0])):
                if (wall_space_obstacle [i][j] != 'S'):
                    if(robot_block[i][j] == 'B'):
                        
                        # LIGNE HU IS DEADLOCK
                        if (isHU  and i == HU):
                            return True
                        # LIGNE HU IS DEADLOCK
                        if (isHD and i == HD):
                        ...
                        # UP RIGHT CORNER
                        if (wall_space_obstacle[i-1][j] =='O' and wall_space_obstacle[i][j+1] == 'O'):
                            return True; 
                        #UP LEFT CORNER
                        if (wall_space_obstacle[i-1][j] =='O' and wall_space_obstacle[i][j-1] == 'O'):
                            return True; 
